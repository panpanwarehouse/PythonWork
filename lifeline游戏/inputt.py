...
class TextBox:
    ...
    def draw(self, dest_surf):
        ...
        # 绘制背景色
        bg=pygame.image.load("input.png")
        pp=pygame.transform.scale(bg,(self.width,self.height))
        # dest_surf.blit(self.__surface, (self.x, self.y))
        dest_surf.blit(pp, (self.x, self.y))
        # 绘制文字
        dest_surf.blit(text_surf, (self.x+25,self.y+self.height//2-20))
        # 绘制联想词
        if self.state == 1:
            dest_surf.blit(self.word_list_surf,
                           (self.x, self.y + (self.height - text_surf.get_height()) - self.height),
